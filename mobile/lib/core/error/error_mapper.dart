...
import 'network_error_types_stub.dart'
    if (dart.library.io) 'network_error_types_io.dart';
import 'result.dart';
...
class ErrorMapper {
  ...
  static AppException fromHttpResponse(
    ...
    return switch (statusCode) {
      ...
      404 => NotFoundException(message: message ?? 'Element non trouve'),
      409 => ConflictException(message: message ?? 'Cet element existe deja'),
  ...
  /// Chaque code fournisseur conserve un message utilisateur distinct.
  static AiException _mapAiError(Map<String, dynamic>? body, String code) {
    ...
    return switch (code) {
      'AI_KEY_INVALID' => AiException(
          code: 'AI_KEY_INVALID',
          provider: provider,
          message: 'Le service IA est mal configure. Contactez le support.',
        ),
      ...
      'AI_PROVIDER_DOWN' => AiException(
          ...
          provider: provider,
          message:
              'Service IA temporairement indisponible. Vous pouvez continuer en mode degrade.',
        ),
  ...
  static AppException fromException(Object error) {
    ...
    return ServerException(message: error.toString(), originalError: error);
  }
  ...
  static AppException _mapValidationOrBusiness(
    ...
    if (code == 'DUPLICATE_EMAIL') {
      return ConflictException(
        message: message ?? 'Cet email est deja utilise',
      );
    }
