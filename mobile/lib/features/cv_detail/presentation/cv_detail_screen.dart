...
import '../../cv/presentation/controllers/cv_detail_controller.dart' as cvp;
...
///
/// Orchestrateur mince : le chargement passe par le [CvDetailController] du
/// domaine CV (Track 2, #240), l'enhancement par [CvEditorController], les
/// exports par le [CvDetailController] local (B3, garde double-clic + erreurs
/// typees).
...
class _CvDetailScreenState extends State<CvDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<cvp.CvDetailController>().load(widget.cvId);
    });
  }
  ...
  Future<void> _openEnhancement(Cv cv, {bool proofreadOnly = false}) async {
    ...
    final ok = await editor.applyEnhancedCv(cv.id!, result);
    if (!mounted) return;
    messenger.showSnackBar(SnackBar(
      content: Text(ok
  ...
  Widget build(BuildContext context) {
    final l = AppLocalizations.of(context)!;
    return Consumer<CvStore>(
      builder: (context, store, _) {
        final cv = store.currentCv;
        if (store.state.isLoading || cv == null) {
          return Scaffold(
