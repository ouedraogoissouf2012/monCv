import 'dart:convert';
import 'package:shared_preferences/shared_preferences.dart';
import '../models/cv.dart';
import 'cv_repository.dart';

const _kCacheKey = 'cached_cvs';

class CachedCvRepository implements CvRepository {
  final CvRepository _remote;
  final SharedPreferences _prefs;

  CachedCvRepository({required CvRepository remote, required SharedPreferences prefs})
      : _remote = remote,
        _prefs = prefs;

  // ── Cache helpers ──────────────────────────────────────────────

  List<Cv>? _readCache() {
    final raw = _prefs.getString(_kCacheKey);
    if (raw == null) return null;
    final list = jsonDecode(raw) as List<dynamic>;
    return list.map((e) => Cv.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<void> _writeCache(List<Cv> cvs) async {
    await _prefs.setString(_kCacheKey, jsonEncode(cvs.map((c) => c.toJson()).toList()));
  }

  Future<void> clearCache() async {
    await _prefs.remove(_kCacheKey);
  }

  // ── CvRepository ──────────────────────────────────────────────

  @override
  Future<List<Cv>> getAllCvs() async {
    try {
      final cvs = await _remote.getAllCvs();
      await _writeCache(cvs);
      return cvs;
    } catch (_) {
      final cached = _readCache();
      if (cached != null) return cached;
      rethrow;
    }
  }

  @override
  Future<Cv> getCvById(int id) => _remote.getCvById(id);

  @override
  Future<Cv> createCv(Cv cv) async {
    final created = await _remote.createCv(cv);
    final cached = _readCache();
    if (cached != null) await _writeCache([...cached, created]);
    return created;
  }

  @override
  Future<Cv> updateCv(int id, Cv cv) async {
    final updated = await _remote.updateCv(id, cv);
    final cached = _readCache();
    if (cached != null) {
      await _writeCache([
        for (final c in cached) c.id == id ? updated : c,
      ]);
    }
    return updated;
  }

  @override
  Future<void> deleteCv(int id) async {
    await _remote.deleteCv(id);
    final cached = _readCache();
    if (cached != null) {
      await _writeCache(cached.where((c) => c.id != id).toList());
    }
  }

  @override
  Future<Cv> duplicateCv(int id) async {
    final copy = await _remote.duplicateCv(id);
    final cached = _readCache();
    if (cached != null) await _writeCache([...cached, copy]);
    return copy;
  }
}
